...
def worker(job_queue):
  while True:
    ...
    # For data generated by the selfplay, not summaries, but details for each match.
...
def self_generate(model_name, model_load_paths):
  random.seed(1)
  models_directory = f'{OUTPUT_PATH}/models/{model_name}'
  model_load_paths.sort()

  # For summaries on who won in which match.
